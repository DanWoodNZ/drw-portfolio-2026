...
export function NextProject({ slug, imageSrc, title, companyName }: NextProjectProps) {
  return (
    <div className="w-full px-4 md:px-8 xl:px-[12%] pt-10 pb-20">
      <Link 
        href={`/projects/${slug}`}
        className="group relative flex flex-col lg:flex-row items-start lg:items-center justify-between p-4 sm:p-6 lg:py-8 lg:pl-8 lg:pr-16 rounded-[32px] overflow-hidden transition-colors duration-500 bg-transparent hover:bg-foreground/5 border border-foreground/10 gap-10 lg:gap-24"
      >
        <div className="flex items-center w-full lg:w-auto shrink-0">
            <div 
              style={{ viewTransitionName: `project-thumb-${slug}` }}
              className="w-full lg:w-[400px] xl:w-[480px] aspect-video rounded-[24px] md:rounded-3xl overflow-hidden relative bg-[#0A0A0A] shrink-0"
            >
              {imageSrc ? (
